...
print_results() {
    $DAEDALUS --domain $DOMAIN --parameters "$1" "$2" --semantics "$3"  --strategy "$4" --info
...
for (( doors_no = N_MIN_DOORS; doors_no <= N_MAX_DOORS; doors_no++ )); do
    for (( tigers_no = N_MIN_TIGERS; tigers_no < doors_no; tigers_no++ )); do
        ...
        print_results "$doors_no" "$tigers_no" kripke  bounded
        print_results "$doors_no" "$tigers_no" delphic unbounded
        print_results "$doors_no" "$tigers_no" kripke  unbounded
    done
done
